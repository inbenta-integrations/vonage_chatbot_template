...
class NexmoConnector extends ChatbotConnector
{
    ...
    protected function isOnlyChat()
    {
        ...
        foreach ($extraInfoData->results as $element) {
            if ($element->name == 'settings') {
                ...
                error_log('only chat from extra info ' . json_encode($onlyChatMode));
                break;
            }
        }
        // Get data from configuration file if it has not been set on ExtraInfo
        // if (is_null($onlyChatMode)) {
    ...
    {
        error_log('Event recived: ' . json_encode($digestedRequest));
        ...
        if ($this->chatOnGoing()) {
            if ($this->isCloseChatCommand($digestedRequest)) {
                ...
                //TODO refactor, the constant is not set in the parent
    ...
    // TODO make it protected in the parent
    /**
